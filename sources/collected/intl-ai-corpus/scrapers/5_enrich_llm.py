#!/usr/bin/env python3
"""Extract structured data from job descriptions using Claude (Anthropic).

Pour chaque YAML brut produit par 4_extract_yaml.py, envoie la description complete
a Claude Sonnet 4.5 qui en extrait :
  - Classification du role : ai-first / ai-support / ml-first (avec raisonnement)
  - Skills categorises en 10 dimensions (genai, ml, web, databases, data, cloud,
    ops, languages, domains, other)
  - Liste des responsibilities extraites
  - Liste des use_cases business mentionnes
  - Flags is_customer_facing + is_management

Output : data_structured/{YYYY-MM}/<id>_<co>_<title>.yaml

Pre-requis : variable d'env ANTHROPIC_API_KEY (https://console.anthropic.com).
"""
import os
import csv
import yaml
...
# Modele LLM utilise (Claude Sonnet 4.5)
...
"""
    ...
    max_attempts = int(os.getenv("LLM_MAX_EXTRACTION_RETRIES", "8"))
    for attempt in range(max_attempts):
        try:
            ...
            # Parse the tool output - Claude may return nested JSON strings
